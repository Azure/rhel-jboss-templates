...
if [ "${CONFIGURATION_MODE}" != "managed-domain" ]; then
    ...
    for ((i = 0; i < NUMBER_OF_INSTANCE; i++)); do
        ...
        --version 2.0 \
        --settings "{\"fileUris\": [\"${standaloneScriptUri}\", \"${postgresqlDSScriptUri}\", \"${mssqlserverDSScriptUri}\", \"${oracleDSScriptUri}\"]}" \
    ...
    az vm extension set --verbose --name CustomScript \
        ...
        --publisher Microsoft.Azure.Extensions \
        --version 2.0 \
        --settings "{\"fileUris\": [\"${masterScriptUri}\", \"${enableElytronSe17DomainCliUri}\", \"${postgresqlDSScriptUri}\", \"${mssqlserverDSScriptUri}\", \"${oracleDSScriptUri}\"]}" \
    ...
    for ((i = 1; i < NUMBER_OF_INSTANCE; i++)); do
        ...
        --publisher Microsoft.Azure.Extensions \
        --version 2.0 \
        --settings "{\"fileUris\": [\"${slaveScriptUri}\", \"${enableElytronSe17DomainCliUri}\", \"${postgresqlDSScriptUri}\", \"${mssqlserverDSScriptUri}\", \"${oracleDSScriptUri}\"]}" \
