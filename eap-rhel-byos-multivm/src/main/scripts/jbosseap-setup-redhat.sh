...
if [ "${CONFIGURATION_MODE}" != "managed-domain" ]; then
    ...
    az vm extension set --verbose --name CustomScript \
        ...
        --protected-settings "{\"commandToExecute\":\"sh jbosseap-setup-master.sh ${JBOSS_EAP_USER} ${JBOSS_EAP_PASSWORD_BASE64} ${RHSM_USER} ${RHSM_PASSWORD_BASE64} ${EAP_POOL} ${RHEL_POOL} ${JDK_VERSION} ${STORAGE_ACCOUNT_NAME} ${CONTAINER_NAME} ${STORAGE_ACCESS_KEY} ${privateEndpointIp} ${CONNECT_SATELLITE} ${SATELLITE_ACTIVATION_KEY_BASE64} ${SATELLITE_ORG_NAME_BASE64} ${SATELLITE_VM_FQDN} ${gracefulShutdownTimeout} ${enablePswlessConnection} ${uamiClientId}\"}"
    ...
    az vm extension set --verbose --name CustomScript \
        ...
        --publisher Microsoft.Azure.Extensions \
        --version 2.0 \
        --settings "{\"fileUris\": [\"${unwrapped_artifactsLocation}${PATH_TO_SCRIPT}/deploy-sample-app.sh\"]}" \
