...
if [[ -z "${STORAGE_ACCESS_KEY}" ]] ; then echo "Failed to get storage account sas token" | log; exit 1; fi
...
if [ "${CONFIGURATION_MODE}" != "managed-domain" ]; then
    ...
    for ((i = 0; i < NUMBER_OF_INSTANCE; i++)); do
        ...
        echo "Set private ip allocation method to Static for host: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}
        ...
        echo "Configure standalone host: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}
        ...
        if [ $? != 0 ] ; then echo "Failed to configure standalone host ${VM_NAME_PREFIX}${i}" | log; exit 1; fi
        echo "standalone ${VM_NAME_PREFIX}${i} extension execution completed" | log; flag=${PIPESTATUS[0]}
    done
else
    ...
    if [[ -z "${privateEndpointId}" ]] ; then echo "Failed to get private endpoint ID" | log; exit 1; fi

    privateEndpointIp=$(az network private-endpoint show --ids $privateEndpointId --query customDnsConfigs[0].ipAddresses[0] -o tsv)
    if [[ -z "${privateEndpointIp}" ]] ; then echo "Failed to get private endpoint IP" | log; exit 1; fi
    ...
    if [[ -z "${DOMAIN_CONTROLLER_PRIVATE_IP}" ]] ; then echo "Failed to get domain controller host private IP" | log; exit 1; fi

    # Update the IP configuration of network interface and set its private ip allocation method to Static
    ipConfigName=$(az network nic show -g ${RESOURCE_GROUP_NAME} -n ${NIC_NAME}0 --query 'ipConfigurations[0].name' -o tsv)
    echo "Set private ip allocation method to Static for host: ${ADMIN_VM_NAME}" | log; flag=${PIPESTATUS[0]}
    ...
    echo "Configure domain controller host: ${ADMIN_VM_NAME}" | log; flag=${PIPESTATUS[0]}
    ...
    az vm extension set --verbose --name CustomScript \
        ...
        # error exception
        if [ $? != 0 ] ; then echo "Failed to configure domain controller host: ${ADMIN_VM_NAME}" | log; exit 1; fi
        echo "Domain controller VM extension execution completed" | log; flag=${PIPESTATUS[0]}
    ...
    for ((i = 1; i < NUMBER_OF_INSTANCE; i++)); do
        ...
        echo "Configure domain slave host: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}
        az vm extension set --verbose --name CustomScript \
        --resource-group ${RESOURCE_GROUP_NAME} \
        --vm-name ${VM_NAME_PREFIX}${i} \
        --publisher Microsoft.Azure.Extensions \
        ...
        if [ $? != 0 ] ; then echo "Failed to configure domain slave host: ${VM_NAME_PREFIX}${i}" | log; exit 1; fi
        echo "Slave ${VM_NAME_PREFIX}${i} extension execution completed" | log; flag=${PIPESTATUS[0]}
    done

    if [ "$ENABLE_DB" == "True" ]; then
        # Configure data source
        echo "Configure data source for host: ${ADMIN_VM_NAME}" | log; flag=${PIPESTATUS[0]}
        ...
        az vm extension set --verbose --name CustomScript \
            ...
            if [ $? != 0 ] ; then echo  "Failed to configure data source for host: ${ADMIN_VM_NAME}" | log; exit 1;  fi
            echo "Data source configuration VM extension execution completed" | log; flag=${PIPESTATUS[0]}

        # Test data source connection in worker nodes
        for ((i = 1; i < NUMBER_OF_INSTANCE; i++)); do
            echo "Test data source connection in workder node: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}
            ...
            if [ $? != 0 ] ; then echo  "Test data source connection failed for worker node: ${VM_NAME_PREFIX}${i}" | log; exit 1;  fi
            echo "Worker node ${VM_NAME_PREFIX}${i} data source connection test extension execution completed" | log; flag=${PIPESTATUS[0]}
        done
    fi    
fi
