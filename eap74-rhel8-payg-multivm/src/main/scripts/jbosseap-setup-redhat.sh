...
if [ "${CONFIGURATION_MODE}" != "managed-domain" ]; then
    ...
    if [[ -z "${privateEndpointId}" ]] ; then echo "Failed to get private endpoint ID" | log; flag=${PIPESTATUS[0]}; exit 1; fi

    privateEndpointIp=$(az network private-endpoint show --ids $privateEndpointId --query customDnsConfigs[0].ipAddresses[0] -o tsv)
    if [[ -z "${privateEndpointIp}" ]] ; then echo "Failed to get private endpoint IP" | log; flag=${PIPESTATUS[0]}; exit 1; fi
    ...
    if [[ -z "${DOMAIN_CONTROLLER_PRIVATE_IP}" ]] ; then echo "Failed to get domain controller host private IP" | log; flag=${PIPESTATUS[0]}; exit 1; fi
    ...
    az vm extension set --verbose --name CustomScript \
        ...
        # error exception
        if [ $? != 0 ] ; then echo "Failed to configure domain controller host: ${ADMIN_VM_NAME}" | log; flag=${PIPESTATUS[0]}; exit 1; fi
    ...
    for ((i = 1; i < NUMBER_OF_INSTANCE; i++)); do
        ...
        if [ $? != 0 ] ; then echo "Failed to configure domain slave host: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}; exit 1; fi
    ...
    if [ "$ENABLE_DB" == "True" ]; then
        ...
        az vm extension set --verbose --name CustomScript \
            ...
            if [ $? != 0 ] ; then echo  "Failed to configure data source for host: ${ADMIN_VM_NAME}" | log; flag=${PIPESTATUS[0]}; exit 1;  fi
        ...
        for ((i = 1; i < NUMBER_OF_INSTANCE; i++)); do
            ...
            if [ $? != 0 ] ; then echo  "Test data source connection failed for worker node: ${VM_NAME_PREFIX}${i}" | log; flag=${PIPESTATUS[0]}; exit 1;  fi
